...
Simulator is an independent fork of [Craft Agents](https://github.com/craft-ai-agents/craft-agents-oss), focused on a locally controlled desktop workspace for running and supervising coding agents.

## How it Works (Upstream Video)

This upstream demo shows the core interaction model inherited by Simulator.
...
Simulator provides an independent product identity and development path while retaining the open-source agent workspace foundation from Craft Agents. It enables intuitive multitasking, direct connections to APIs and services, session sharing, and a document-centric workflow in a desktop UI.
...
项目政策：[隐私政策](PRIVACY.md) · [支持与版本政策](SUPPORT.md) · [详细版本政策](docs/VERSIONING.md) · [架构决策](docs/adr/README.md) · [安全政策](SECURITY.md) · [贡献指南](CONTRIBUTING.md)
...
Simulator connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
Simulator can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
A terminal client that connects to a running Simulator server over WebSocket. Use `wss://` for remote servers; `ws://` is only for loopback development connections. Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
4. Name: e.g., "Simulator Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Simulator
...
Simulator supports multiple ways to connect to LLM providers:
...
Simulator uses two agent backends:
...
**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\Simulator\Simulator.exe" -- --debug
```
