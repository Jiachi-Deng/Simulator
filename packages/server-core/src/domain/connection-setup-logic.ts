...
export function parseTestConnectionError(msg: string): string {
  ...
  if (lower.includes('no api key found for')) {
    return 'Provider mismatch during setup. Select a provider preset in Simulator Backend API Key mode, or use Anthropic API Key mode for arbitrary compatible endpoints.'
  }
...
export function validateSetupTestInput(params: {
  ...
  if (params.provider === 'pi' && hasCustomEndpoint && !params.piAuthProvider) {
    return {
      valid: false,
      error: 'Custom endpoint in Simulator Backend mode requires selecting a provider preset. For arbitrary Anthropic-compatible endpoints, use Anthropic API Key mode.',
    }
  }
...
export const BUILT_IN_CONNECTION_TEMPLATES: Record<string, {
  ...
  'pi-api-key': {
    name: 'Simulator Backend (API Key)',
    providerType: 'pi',
    authType: 'api_key',
