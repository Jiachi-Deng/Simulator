...
import type { ModuleReleaseEnvelopeV1, TrustedReleaseKey } from './types.ts'
...
describe('verifyModuleReleaseCatalog', () => {
  ...
  it('rejects rollback and replay at or below the monotonic high-water mark', () => {
    ...
    expect(diagnosticCodes(verify(envelope, [trustedKey], { state: { highestSequence: 7 } }))).toEqual(['ROLLBACK_DETECTED'])
    expect(diagnosticCodes(verify(envelope, [trustedKey], { state: { highestSequence: 8 } }))).toEqual(['ROLLBACK_DETECTED'])
  })
  ...
  it('returns structured diagnostics with stage, JSON pointer path, and key ID', () => {
    ...
    const result = verify(envelope, [trustedKey], { state: { highestSequence: 1 } })
