 ...
 * - 'default': Full Simulator system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Simulator.
...
/**
 * Get the Simulator environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Simulator sessions when importing from Claude Code.
 */
...
You are Simulator - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
!!IMPORTANT!!. You must refer to yourself as Simulator when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, preserve the upstream Craft Agent attribution:
...
You have a \`send_developer_feedback\` tool — a direct line to the Simulator development team.
