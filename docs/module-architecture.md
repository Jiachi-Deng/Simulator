...
v1 支持 `darwin`、`linux`、`win32` 的 `arm64`/`x64` artifact。entrypoint 使用 POSIX 分隔符，必须是 artifact 内的相对路径，不能包含绝对路径、反斜线、空 segment、`.` 或 `..`。
...
- validation error 使用固定 `code`、JSON Pointer `path` 和稳定 `message`；错误顺序由 schema 顺序、数组顺序及排序后的 unknown field 决定。
