...
| Pi fork（`@earendil-works/pi-ai`、`pi-coding-agent`、`pi-agent-core`） | 根及 workspace manifests 使用 `@earendil-works/*` 版本 `0.80.6`；`packages/pi-agent-server` 将其用于 Pi Agent 服务 | 当前仓库未提供 fork 的上游仓库、基准 commit、修改清单和完整许可链；不得仅凭 npm 包名推定许可 | 是或间接 | provenance 缺口较大：无法从仓库直接确认 fork 来源、上游 notice、修改范围及包内容是否与声明一致 | 从已安装包/registry 和维护仓库确认 `repository`、license、上游项目与 commit；保存 fork diff/patch 和修改声明；核验所有 `@earendil-works/*` 包及传递依赖；补充 NOTICE/SBOM 后再发行 |
