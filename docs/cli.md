...
Terminal client for Simulator server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running Simulator headless server with URL and token
