 ...
 *
 * Warns against using localStorage in Simulator codebase.
 * All persistent user settings should be stored in file-based configs
 * (preferences.json, workspace configs) for consistency with Simulator
...
module.exports = {
  meta: {
    ...
    messages: {
      noLocalStorage:
        "Avoid localStorage in Simulator. Store settings in ~/.craft-agent/preferences.json using window.electronAPI.readPreferences/writePreferences. See packages/shared/src/config/preferences.ts for the preferences API.",
    },
    schema: [],
  },
