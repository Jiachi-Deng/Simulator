...
This guide explains how to configure automations in Simulator to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in Simulator. You can:
...
### App Events (triggered by Simulator)
...
Send a prompt to Simulator (creates a new session for scheduled prompts).
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from Simulator!" }
}
```
...
4. **Pair the supergroup.** In Simulator: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.
...
Ask Simulator to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and Simulator was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
