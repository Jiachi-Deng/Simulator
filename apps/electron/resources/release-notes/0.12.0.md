...
- **OpenDesign 0.14.6 RC reuses the Craft Agent Runtime through a standard CLI contract** — The current prerelease, OpenDesign `0.14.6-rc.1`, launches the Host-owned `simulator-host-agent` as an ordinary `json-event-stream` Runtime, with one transient Craft Session per Turn, no OpenDesign Cloud/AMR login, no external CLI setup, and no provider/model selector inside the Module. v1 compatibility for OpenDesign `0.14.5` remains available for rollback. Issue #129.
