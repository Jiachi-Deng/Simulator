...
describe('OpenDesign RC acceptance evidence workflow', () => {
  ...
  test('authenticates exact LKG and RC bytes, signed Catalog high-water, and machine trust closure', () => {
    ...
    expect(release).toContain('SOURCE_AUTHORITY_CONFIG="$SOURCE_AUTHORITY_CONFIG" LKG_AUTHORITY_CONFIG="$lkg/$LKG_CONFIG_ASSET"')
    expect(release).toContain('version: "0.14.6"')
    expect(release).toContain('version: "0.14.5"')
    expect(release).toContain('Future stable and LKG trust roots differ')
    expect(release).not.toContain('cmp "$SOURCE_AUTHORITY_CONFIG" "$lkg/$LKG_CONFIG_ASSET"')
    ...
    expect(release).toContain('.h1Authority.connectionEvidenceSha256 "$manifest"')
    expect(release).toContain('.h1Authority.handoffSha256 "$manifest"')
    expect(release).toContain('connectionEvidenceSha256: $h1ConnectionEvidenceSha256')
    expect(release).toContain('handoffSha256: $h1HandoffSha256')
