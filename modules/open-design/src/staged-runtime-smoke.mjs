...
export async function smokeStagedRuntime({ artifactRoot, nodeBin, timeoutMs = 60_000, spawnProcess = spawn } = {}) {
  ...
  const runtimeParent = path.dirname(artifactRoot);
  const runtimeRoot = await mkdtemp(path.join(runtimeParent, ".open-design-smoke-")).catch((error) => stagingFail("SMOKE_RUNTIME_INVALID", error.message));
