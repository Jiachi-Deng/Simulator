...
3. 真实模式按 upstream `tools/pack` 的 build closure 顺序执行 `pnpm install --frozen-lockfile`、workspace runtime build、Next standalone、web sidecar 和两个 `pnpm deploy --prod --legacy` closure；`--legacy` 是 pnpm 10 对未启用 injected workspace packages 的显式 deploy 兼容模式。
