...
6. native inventory 对最终 staged `better-sqlite3` 和 `node-pty` 的二进制格式、platform、arch、Node ABI、libc 和 producer metadata 做闭包检查。上游 `pnpm.onlyBuiltDependencies` 仍必须允许 build 阶段使用的 `better-sqlite3`、`node-pty` 和 `sharp`；`node-pty` 被 ignored 时立即失败。Host-only Next 使用 unoptimized images，因此 `sharp` 及完整 `@img` optimizer closure 在最终 staging 前被删除，不是 Artifact 必需 native package。
